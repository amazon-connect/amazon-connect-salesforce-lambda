...
def createChatterPost(sf, sf_object, sf_feedElementType, sf_subjectId, sf_messageType, sf_message, **kwargs):
  for key, value in kwargs.items():
    logger.info("Replacing [%s] with [%s] in [%s]" % (key, value, sf_message))
    sf_message = sf_message.replace(key, value)
  ...
  return {'Id': sf.createChatterPost(sobject=sf_object, data=data)}


def createChatterComment(sf, sf_object, sf_feedElementId, sf_commentType, sf_commentMessage, **kwargs):
  for key, value in kwargs.items():
    logger.info("Replacing [%s] with [%s] in [%s]" % (key, value, sf_commentMessage))
    sf_commentMessage = sf_commentMessage.replace(key, value)

  data = {'sf_feedElementId': sf_feedElementId,
          'sf_commentType': sf_commentType,
          'sf_commentMessage': sf_commentMessage}

  return {'Id': sf.createChatterComment(sobject=sf_object, sfeedElementId=sf_feedElementId, data=data)}
